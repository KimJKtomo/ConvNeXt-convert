# filename: train_ddp_fracture_per_agegroup_convnextv2.py
import os
import pandas as pd
import numpy as np
import torch
...
# ✅ 데이터 준비
df_train_raw = pd.read_csv("age_train_tmp.csv")
df_val_raw = pd.read_csv("age_val_tmp.csv")
df_all = pd.concat([df_train_raw, df_val_raw]).reset_index(drop=True)

df_all["label"] = df_all["fracture_visible"].apply(lambda x: 1.0 if x == 1.0 else 0.0)
df_all["fracture_visible"] = df_all["fracture_visible"].fillna(0)
...
for group in sorted(df_all["age_group_label"].unique()):
    df_g = df_all[df_all["age_group_label"] == group]
    splitter = StratifiedShuffleSplit(n_splits=1, test_size=0.3, random_state=42)
    for train_idx, val_idx in splitter.split(df_g, df_g["label"]):
        df_train_split.append(df_g.iloc[train_idx])
        df_val_split.append(df_g.iloc[val_idx])

df_train = pd.concat(df_train_split).reset_index(drop=True)
df_val = pd.concat(df_val_split).reset_index(drop=True)
...
for age_group in [0, 1, 2, 3]:
    ...
    if count_1 == 0:
        ...
        if imbalance_ratio > 5.0 and minority_count >= 50:
    ...
    if local_rank == 0:
        mlflow.start_run(run_name=f"ConvNeXtV2_AgeGroup{age_group}")
    ...
    for epoch in range(EPOCHS):
        ...
        if local_rank == 0:
            ...
            print(f"[Train] Epoch {epoch} | Loss: {total_loss:.4f} | Acc: {acc:.4f} | F1: {f1:.4f}")
        ...
        if local_rank == 0:
            ...
            if epoch == 0 or val_f1 > best_f1:
                best_f1 = val_f1
                best_model_path = f"0722_best_ddp_convnextv2_agegroup{age_group}.pt"
